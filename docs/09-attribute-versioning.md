...
每个 `(root_type, root_id, attr_key, seq_no)` 同一时刻最多一个当前行（`valid_to IS NULL`）。
DB 层用一个**生成列 + 唯一索引**实现：
...
`seq_no` 的加入让多值 attribute（`is_multi=1`）的多个值各自独立唯一：
- 单值 (`is_multi=0`)：所有当前行 `seq_no=0`，等价于"按 attr_key 唯一"
- 多值 (`is_multi=1`)：每个 `seq_no` 各自独立唯一（不同 `seq_no` 之间不冲突）

MySQL 唯一索引中 `NULL` 不参与去重 → 历史行（`current_flag=NULL`）可多条共存，
当前行（`current_flag=1`）全局唯一。写入时若忘记关旧行，DB 直接报 1062 错误。

> **写入顺序约束**：`uk_current` 强依赖"先关旧、再开新"（§4.1）。  
> 若某路径在开新前没关旧，`INSERT` 会因 `uk_current` 冲突失败 —— 这就是 DB 兜底。  
> 单值与多值必须走**同一个**关旧 SQL（限定 `seq_no`），不能用一个不带 `seq_no` 的 UPDATE 去关多值的多个 seq_no。
...
- **废弃 attribute**（默认行为，**软删**）：
  1. `UPDATE attr_def SET deprecated_at = NOW(3) WHERE (root_type, attr_key) = ...`（schema 标记废弃）
  2. 关闭该 `(root_type, attr_key)` 下**所有**聚合根的当前行：
     `UPDATE attr_value SET valid_to = NOW(3) WHERE ... AND valid_to IS NULL`
  3. `DELETE FROM attr_value_current WHERE ...`
  4. **`attr_value` 历史行保留**——仅 schema 标记废弃，仍可用于审计/历史回溯
- **物理删除**（仅合规/隐私"被遗忘权"场景，慎用）：
  1. 先关当前行（`UPDATE attr_value SET valid_to = NOW(3) WHERE ... AND valid_to IS NULL`）
  2. 删 `attr_value_current` 对应行
  3. 再 `DELETE FROM attr_value WHERE ...`（历史已闭的行才删，已关当前行的 `valid_to` 非空）
...
| 来源 | 事件模式 | 含义 |
|---|---|---|
| `attr_value` | `UPDATE`，`valid_to` 由 `NULL → 非空` | 旧版本结束（属性变更） |
| `attr_value` | `INSERT` 且 `valid_to IS NULL` | 新当前值 |
| `attr_value` | `UPDATE`，`valid_to` 由 `NULL → 非空` **且**无对应 `INSERT` | 属性作废（对应 `attr_value_current` 同步 `DELETE`） |
| `attr_value_current` | `DELETE` | 当前行消失（与上一种是同一事件的两侧） |
| `attr_def` | `UPDATE deprecated_at` | 整条 attribute 废弃（schema 事件） |

> **合并/批量作废的 binlog 风暴**：合并 A→B 时一次性 UPDATE A 的所有当前行 + DELETE+重建 B 的 current，
> 单次合并可触发数千条 binlog 事件。下游消费者需做**批处理/限流**，否则峰值时会被打挂。
> 建议在 master-service 端用 `binlog_event_merger` 合并"同一 root_id 在 100ms 窗口内"的事件。
